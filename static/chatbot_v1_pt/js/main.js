...
async function server_chat(userMessage) {
    ...
    const fetchMessageStream = async () => {
        ...
        const response = await fetch("/chatbot_v1_pt/get_message_stream/", {
            method: "POST",
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
                'X-CSRFToken': csrfToken
    ...
    //서버통신
    const response = await fetch("/singgo/", {
        method: "POST",
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
            'X-CSRFToken': csrfToken
    ...
    if (response.ok) {
        ...
        //스트림 데이터 초기화
        const response2 = await fetch("/chatbot_v1_pt/get_message_stream_default/", {
            method: "POST",
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
                'X-CSRFToken': csrfToken
