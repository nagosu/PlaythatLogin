...
//체크 라디오 버튼 가져오기
    ...
    async function data_list(page_number){
        ...
        // 요청 보내기
        const response = await fetch("/chatbot_v1_pt/admin/data_list_log/", {
            method: "POST",
            headers: {
    ...
    function updateTable(data) {
        ...
        document.querySelectorAll('table td').forEach(cell => {
            cell.addEventListener('click', function() {
                ...
                modalBody.textContent = "선택된 정보: " + this.textContent; // 셀의 텍스트를 모달에 표시
                $('#infoModal').modal('show'); // Bootstrap의 jQuery 함수를 사용하여 모달을 표시
            });
        });
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        modal.querySelector('.btn-primary').addEventListener('click', async function() {
            ...
            // 여기에 실제 삭제 처리 로직
            // 요청 보내기
            const response = await fetch("/chatbot_v1_pt/admin/data_list_image_delete/", {
                method: "POST",
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'X-CSRFToken': csrfToken
