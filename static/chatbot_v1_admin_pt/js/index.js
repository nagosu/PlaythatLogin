...
//카테고리 불러오기
        ...
        const response = await fetch("/chatbot_v1_pt/admin/category_list/", {
            method: "POST",
            headers: {
        ...
        if (response.ok) {
            console.log("/chatbot_v1_pt/admin/category_list/")
            /* 데이터 가져오기 */
            ...
            for(var i=0; i<category_line_container_list.length; i++) {
                ...
                //카테고리 URL 값을 넣는다
                //getData 값을 넣는다
                category_line_container_list[i].querySelector('.category_url').value = "http://3.34.108.56/chatbot_v1_pt/";
...
//카테고리에서 수정 버튼 클릭시
            ...
            document.addEventListener('click', async function(event) {
              ...
              if (event.target.classList.contains('category_edit')) {
                ...
                /* 응답 요청 await 함수는 async 내에서만 사용 가능*/
                const response = await fetch("/chatbot_v1_pt/admin/category_data/", {
                    method: "POST",
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                        'X-CSRFToken': csrfToken
            ...
            document.addEventListener('click', async function(event) {
                if (event.target.classList.contains('btn-outline-danger')) { // '삭제' 버튼 클릭 시
                    ...
                    document.getElementById('btn_confirm_category_delete').addEventListener('click', async function() {
                        ...
                        /* 응답 요청 await 함수는 async 내에서만 사용 가능*/
                        const response = await fetch("/chatbot_v1_pt/admin/category_delete/", {
                            method: "POST",
                            headers: {
                                'Content-Type': 'application/x-www-form-urlencoded',
                                'X-CSRFToken': csrfToken
...
//전송 버튼 눌렀을 때
    document.querySelector(".confirm_button").addEventListener("click", async(event) => {
        ...
        /* 응답 요청 await 함수는 async 내에서만 사용 가능*/
        const response = await fetch("/chatbot_v1_pt/admin/send_data/", {
            method: "POST",
            headers: {
                'X-CSRFToken': csrfToken
            },
